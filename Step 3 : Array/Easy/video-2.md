...
### Incorrect attempt (swap chain)

You may be tempted to implement rotation by repeatedly swapping adjacent elements as shown below. This code does perform a transformation similar to rotation, but it's less clear and performs more writes than the optimal approach:
...
- Unclear intent: The repeated swaps hide the fact that you want a rotation. A reader expects a direct shift or rotate, not a chain of swaps.

- More writes: Each swap does three assignments. With `n-1` swaps that’s ~3(n-1) writes, whereas the `temp`+shift uses `n` writes (n-1 shifts + 1 write), so the swap-chain performs significantly more element writes.

- Poor generalization: The swap-chain only mimics a single left rotation and scales badly for rotating by `d` places; it’s not a clean or efficient general solution.
- Harder to maintain: The logic is less explicit and more error-prone than the simple `temp`+shift or the three-reverse method.

Recommendation: Prefer the `temp`+shift method for a single left rotation, or the three-reverse technique for rotating by `d` places — both are clearer, use fewer writes, and are easier to reason about.
...
- **Approach:**
  ...
  3.  Iterate $\mathbf{I}$ from $J+1$ to $N-1$:
      ...
      - **Advance $\mathbf{J}$:** After the swap, $A[J]$ now contains the non-zero element, and $A[J+1]$ contains the zero that was just swapped. Thus, advance $J$ to $J+1$ to point to the _next_ zero element.
