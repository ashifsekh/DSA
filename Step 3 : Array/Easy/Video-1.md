...
**Goal:** Determine if the array is sorted in a non-descending order ($A[i] \ge A[i-1]$).

### A. Approach (Single Pass)

  * **Thinking:** We only need to find a single instance where the current element is *smaller* than the previous element. If we find that, we can immediately stop and return `false`.
  * **Approach:**
    1.  Start iterating from the **second element** ($\mathbf{i=1}$).
    2.  In each step, check if $\mathbf{A[i]} < \mathbf{A[i-1]}$.
    3.  If the condition is met, return `false`.
    4.  If the loop completes without returning `false`, return `true`.
  * **Time Complexity:** $O(N)$. (Best case $O(1)$ if the first check fails).
...
public static boolean isSorted(int[] arr) {
    ...
    for (int i = 1; i < arr.length; i++) {
        // Non-descending means A[i] should be >= A[i-1]. 
        // We look for the violation: A[i] < A[i-1].
        if (arr[i] < arr[i - 1]) {
            return false; // Found an unsorted pair
        }
    }
    return true; // Loop completed, array is sorted
}
```
